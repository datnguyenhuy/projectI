...
int Game::getRoomId(char* mess)
{
	...
	if (strcmp(protocol, "MAPHONG") == 0) {
		if (strcmp(value, "OK") == 0) {
			...
			else if (roomId == 4) return -4; // mat khau sai
	}
	return -5;
}
...
void Game::GD1()
{
	...
	//LOOP nhap ten
	char nameInput[30];// luu ten tam thoi
	bool notEnterName = true; // luu trang thai nhap ten
	bool cannotConnect = true; // luu trang thai ket noi 
...
void Game::GD2()
{
	...
	//LOOP 
	bool notGetRoomId = true;// trang thai lay ma phong
	char s[20], pass[20];
	int room;
...
int Game::gameStart()
{	//debug
	/*this->you = new PlayerOffline("dat", this->server);
	...
	replay.setBold();
	exit.setText(&font, Color::White);
	exit.setStringUnicode(L"Thoát đi");
	exit.reSizeText(25);
	exit.setBold();
